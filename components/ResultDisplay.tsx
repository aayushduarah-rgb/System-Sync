...
import type { CompatibilityReport, Game, SystemSpecs } from '../types';
...
export const ResultDisplay: React.FC<ResultDisplayProps> = ({ result, game, userSpecs }) => {
  const { isCompatible, estimatedFps, analysis, componentCheck } = result;
  ...
  return (
    <section id="result" className="bg-gray-900 bg-opacity-70 backdrop-blur-sm rounded-lg p-6 md:p-8 border border-gray-700 animate-fade-in-up">
        ...
        <div className="text-center mb-8">
            <h3 className="text-3xl font-bold mb-2 text-white">Compatibility Report for <span className="text-cyan-400">{game.name}</span></h3>
            <div className="flex items-center justify-center gap-6 mt-4">
                ...
                <div className="text-center">
                    <div className="text-4xl font-bold text-cyan-400">{estimatedFps}</div>
                    <div className="text-sm uppercase tracking-wider text-gray-300">Est. FPS</div>
                </div>
            </div>
             <p className="mt-4 text-gray-300 max-w-2xl mx-auto text-center">{analysis}</p>
        </div>
        ...
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 max-w-5xl mx-auto">
                ...
                <RequirementCard title="Recommended" specs={game.recReqs} />
                <RequirementCard title="Minimum" specs={game.minReqs} />
            </div>
        </div>
    </section>
  );
